...
def DistanceClassifier(input_size=(32,32,3), num_classes=10):
    ...
    # second conv block
    x = Conv2D(filters=32, kernel_size=(3, 3), padding='same', kernel_regularizer=regularizers.l2(1e-4))(x)
    x = LeakyReLU(alpha=0.2)(x)
    ...
    # 4th conv block
    x = Conv2D(filters=64, kernel_size=(3, 3), padding='same', kernel_regularizer=regularizers.l2(1e-4))(x)
    x = LeakyReLU(alpha=0.2)(x)
    ...
    # 8th conv block
    x = Conv2D(filters=128, kernel_size=(3, 3), padding='same', kernel_regularizer=regularizers.l2(1e-4))(x)
    x = LeakyReLU(alpha=0.2)(x)
